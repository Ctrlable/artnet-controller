...
_STORAGE_KEY='artnet_led_license_cache'
_STORAGE_VERSION=1
...
def validate_license_offline(token,current_version=_A):
	...
	E=A.get('product')
	if EXPECTED_PRODUCT and E!=EXPECTED_PRODUCT:raise LicenseError(f"License is for product {E!r}, not Art-Net ({EXPECTED_PRODUCT!r})")
...
_KEY_STORAGE_KEY='artnet_led_license_key'
	...
	try:
		async with aiohttp.ClientSession()as F:
			async with F.get(E,params=D,timeout=aiohttp.ClientTimeout(total=10))as A:
				if A.status==404:_LOGGER.warning('Ctrlable Art-Net: license JTI %s not found on portal',jti);return _C
				if A.status==200:
					B=await A.json()
					if B.get('instance_mismatch'):_LOGGER.error('Ctrlable Art-Net: license instance mismatch — this license belongs to a different HA installation.');return _C
					return bool(B.get('valid',not B.get('is_revoked',_C)))
				_LOGGER.warning('Ctrlable Art-Net: license check returned HTTP %s',A.status)
	except Exception as G:_LOGGER.debug('Ctrlable Art-Net: portal unreachable (%s) — operating offline',G)
async def periodic_revocation_check(hass,jti,entry_id,instance_id=_A):
	while _B:
		...
		if A is _C:_LOGGER.error('Ctrlable Art-Net: license revoked or instance mismatch. Disabling integration.');await hass.config_entries.async_unload(entry_id);return
